...
type CharInfo =
  struct
    ...
    /// Get or set the shallpass parameter: it defines the wall pixel.
    member this.sp
        with get () = Color.color_of_nibble (this.Attributes >>> 8)
        and set c = this.Attributes <- (this.Attributes &&& 0xf0ffs) ||| (Color.nibble_of_color c <<< 8)

    /// Get or set the option parameter: it defines the option pixel of menu.
    member this.op
        with get () = Color.color_of_nibble (this.Attributes >>> 12)
        and set c = this.Attributes <- (this.Attributes &&& 0x0fffs) ||| (Color.nibble_of_color c <<< 12)
