...
type CharInfo with
    ...
    static member create (c : char, fg : Color, ?bg : Color, ?sp : Color, ?op: Color) = 
        let bg = defaultArg bg Color.Black
        // boolean: White = 1 = true, you can pass ;  Black = 0 = false, you shall not pass
        let sp = defaultArg sp Color.White
        // Color: Black = 0 = no option
        let op = defaultArg op Color.Black
    ...
    /// Shortcut for creating a filled cell given the foreground and background colors with pointer char.
    static member pointer (fg : Color, ?bg : Color) = CharInfo.create (Config.pointer_pixel_char, fg, ?bg = bg) 
    
    /// Shortcut for creating an empty cell given the foreground and background colors.
    static member empty = CharInfo.create (Config.empty_pixel_char, Color.Black) 
...
type system_console_raster (w, h) =
    //MOD: modified h with h+1 to clear menu rectangle changing scene
    inherit wronly_raster (w, h+1)
