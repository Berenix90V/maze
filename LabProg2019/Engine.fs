...
type engine (w : int, h : int, ?fps_cap : int, ?flip_queue) =
    ...
    /// DeRegister the given sprite so that the engine does not render it
    member __.deregister_sprite (spr : sprite) =
        let len = lock sprites <| fun () ->
            sprites <-  List.sortBy (fun spr -> spr.z) (List.filter (fun x -> x <> spr) sprites) // sprites are always sorted in ascending order by z          
            List.length sprites
        Log.msg "deregistered sprite: x=%g y=%g z=%d width=%d height=%d" spr.x spr.y spr.z spr.width spr.height
    ...
    ///shoot1 4 start menu
    /// Plus parameters from loop1
    /// Color :  to know which option has been selected
    /// mw mh = size for main game
    member private this.shoot1 update (data : _ loop_data) =
           let buff = buffers.[data.frame_cnt % Array.length buffers]
           let (st', quit'), color, mw, mh, ts = stopwatch_quiet1 <| fun () ->
               buff.lock_and_commit <| fun wr ->
                   if this.auto_clear then wr.clear
                   let st', quit', color, mw, mh = update wr data.state
                   if this.show_sprites then lock sprites <| fun () -> for spr in sprites do spr.draw wr
                   if this.show_fps then
                       let dt = DateTime.Now - data.now
                       let hd = sprintf "frame count: %d\nframe time: %.1f ms (%.1f fps)" data.frame_cnt data.elapsed.TotalMilliseconds (float data.frame_cnt / dt.TotalSeconds)
                       wr.draw_text (hd, 0, 0, Color.Yellow, Color.Blue)
                   st', quit', color, mw, mh
           { data with state = st'; frame_cnt = data.frame_cnt + 1; quit = quit'; elapsed = ts }, color, mw, mh
    ...
    /// next: an integer tomanage the next scene 0 = quit, 2 = game scene, 1 = same menu
    /// mw mh = width and height to pass to main game
    /// color : useful to know which option has been selected
    member this.loop_on_key1 update st0 =
        Log.msg "entering engine on-key loop..."
        let mutable data = { state = st0; frame_cnt = 0; quit = false; elapsed = new TimeSpan (); now = DateTime.Now }
        let mutable col = Color.Black
        let mutable mw = 21
        let mutable mh = 21
        let new_data, color, mw1, mh1 = this.shoot1 (fun _ st -> (st, false), false, col, mw, mh) data
        data <- new_data
        let mutable next = 1
        let mutable size = mw, mh
        while not data.quit do
            let k = Console.ReadKey true
            // new
            engine.log_key_pressed k
            //Log.debug "engine: key pressed: %c" k.KeyChar  
            let new_data, color, mw1, mh1 = this.shoot1 (update k) data
            data <- new_data
            col <- color
            next <- 1
            mw <- mw1
